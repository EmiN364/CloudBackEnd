...
products.get('/similar/:id', async (c) => {
  try {
    const productId = parseInt(c.req.param('id'));
    ...
    const productResult = await pool.query(
      'SELECT category FROM products WHERE id = $1 AND deleted = false',
      [productId]
    );
    ...
    const result = await pool.query(
      `SELECT p.id, p.name, p.description, p.category, p.price, p.image_url,
       ...
       FROM products p
       LEFT JOIN users u ON p.seller_id = u.id
       WHERE p.category = $1 AND p.id != $2 AND p.deleted = false AND p.paused = false
       ORDER BY p.id DESC
       LIMIT 10`,
      [category, productId]
    );
