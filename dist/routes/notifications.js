...
notifications.get('/', authMiddleware, async (c) => {
    try {
        ...
        const result = await pool.query(`SELECT id, message, type, read, date, product_id
       FROM notifications
       ${whereClause}
       ORDER BY date DESC
       LIMIT $${paramCount} OFFSET $${paramCount + 1}`, values);
        // Get total count
        const countResult = await pool.query(`SELECT COUNT(*) FROM notifications ${whereClause}`, values.slice(0, -2));
        const total = parseInt(countResult.rows[0].count);
        const totalPages = Math.ceil(total / limit);
        // Get unread count
        const unreadCountResult = await pool.query('SELECT COUNT(*) FROM notifications WHERE user_id = $1 AND read = false', [user.id]);
...
notifications.get('/:id', authMiddleware, async (c) => {
    try {
        ...
        const result = await pool.query(`SELECT id, message, type, read, date, product_id
       FROM notifications
       WHERE id = $1 AND user_id = $2`, [notificationId, user.id]);
...
notifications.patch('/:id/read', authMiddleware, async (c) => {
    try {
        ...
        const result = await pool.query('UPDATE notifications SET read = true WHERE id = $1 AND user_id = $2 RETURNING id', [notificationId, user.id]);
        if (result.rows.length === 0) {
            return c.json({ error: 'Notification not found' }, 404);
        }
...
notifications.post('/', authMiddleware, async (c) => {
    try {
        ...
        const userCheck = await pool.query('SELECT id FROM users WHERE id = $1 AND deleted = false', [targetUserId]);
        if (userCheck.rows.length === 0) {
            return c.json({ error: 'Target user not found' }, 404);
        }
        const result = await pool.query(`INSERT INTO notifications (user_id, message, type, product_id)
       VALUES ($1, $2, $3, $4)
...
notifications.put('/:id', authMiddleware, async (c) => {
    try {
        ...
        const notificationCheck = await pool.query('SELECT id FROM notifications WHERE id = $1 AND user_id = $2', [notificationId, user.id]);
        ...
        const result = await pool.query(`UPDATE notifications SET ${updateFields.join(', ')}
       WHERE id = $${paramCount} AND user_id = $${paramCount + 1}
       RETURNING id, message, type, read, date, product_id`, values);
        return c.json({
...
notifications.delete('/read', authMiddleware, async (c) => {
    try {
        const user = c.get('user');
        const result = await pool.query('DELETE FROM notifications WHERE user_id = $1 AND read = true RETURNING id', [user.id]);
