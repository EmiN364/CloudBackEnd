...
sales.get('/', authMiddleware, async (c) => {
    try {
        ...
        const result = await pool.query(`SELECT s.id, s.date, s.total, s.status, s.note, s.invoice_id, s.address
       FROM sales s
       WHERE s.user_id = $1
       ORDER BY s.date DESC
       LIMIT $2 OFFSET $3`, [user.id, limit, offset]);
        // Get total count
        const countResult = await pool.query('SELECT COUNT(*) FROM sales WHERE user_id = $1', [user.id]);
        const total = parseInt(countResult.rows[0].count);
        const totalPages = Math.ceil(total / limit);
        // Get products for each sale
        const salesWithProducts = await Promise.all(result.rows.map(async (sale) => {
            const productsResult = await pool.query(`SELECT sp.product_id, sp.price, sp.amount,
                  p.name, p.description,
                  u.first_name, u.last_name
           FROM sales_products sp
           JOIN products p ON sp.product_id = p.id
           JOIN users u ON p.seller_id = u.id
           WHERE sp.sale_id = $1`, [sale.id]);
            return {
                ...sale,
...
sales.get('/:id', authMiddleware, async (c) => {
    try {
        ...
        const result = await pool.query(`SELECT s.id, s.date, s.total, s.status, s.note, s.invoice_id, s.address
       FROM sales s
       WHERE s.id = $1 AND s.user_id = $2`, [saleId, user.id]);
...
sales.post('/', authMiddleware, async (c) => {
    try {
        ...
        // Get user's cart
        const cartResult = await pool.query(`SELECT cp.product_id, cp.amount, p.price, p.name, p.seller_id
       ...
       WHERE cp.user_id = $1 AND p.deleted = false AND p.paused = false`, [user.id]);
        ...
        try {
            ...
            // Create sale
            const saleResult = await client.query(`INSERT INTO sales (user_id, total, status, note, invoice_id, address)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id`, [
                user.id,
                total,
            ...
            for (const item of cartResult.rows) {
                await client.query('INSERT INTO sales_products (sale_id, product_id, price, amount) VALUES ($1, $2, $3, $4)', [saleId, item.product_id, item.price, item.amount]);
            }
            // Clear cart
            await client.query('DELETE FROM cart_products WHERE user_id = $1', [user.id]);
...
sales.put('/:id/status', authMiddleware, async (c) => {
    try {
        ...
        const saleCheck = await pool.query('SELECT id FROM sales WHERE id = $1 AND user_id = $2', [saleId, user.id]);
...
sales.patch('/:id/cancel', authMiddleware, async (c) => {
    try {
        ...
        const saleCheck = await pool.query('SELECT id, status FROM sales WHERE id = $1 AND user_id = $2', [saleId, user.id]);
        if (saleCheck.rows.length === 0) {
            return c.json({ error: 'Sale not found' }, 404);
        }
        ...
        await pool.query('UPDATE sales SET status = $1 WHERE id = $2', ['cancelled', saleId]);
        return c.json({
