...
stores.get('/', async (c) => {
    try {
        ...
        const result = await pool.query(`SELECT s.store_id, s.store_name, s.description, s.cbu,
       ...
       LIMIT $${paramCount} OFFSET $${paramCount + 1}`, values);
        // Get total count
        const countResult = await pool.query(`SELECT COUNT(*) FROM stores s ${whereClause}`, values.slice(0, -2));
...
stores.get('/:id', async (c) => {
    try {
       ...
       LEFT JOIN images si ON s.store_image_id = si.id
       LEFT JOIN images ci ON s.cover_image_id = ci.id
       WHERE s.store_id = $1 AND u.deleted = false AND u.is_active = true`, [storeId]);
        if (result.rows.length === 0) {
            return c.json({ error: 'Store not found' }, 404);
        }
        // Get store products count
        const productsCountResult = await pool.query('SELECT COUNT(*) FROM products WHERE seller_id = $1 AND deleted = false AND paused = false', [storeId]);
...
stores.post('/', authMiddleware, sellerMiddleware, async (c) => {
    try {
        ...
        const existingStore = await pool.query('SELECT store_id FROM stores WHERE store_id = $1', [user.id]);
        if (existingStore.rows.length > 0) {
            return c.json({ error: 'Store already exists for this user' }, 409);
        }
        const body = await c.req.json();
        const validatedData = createStoreSchema.parse(body);
        const result = await pool.query(`INSERT INTO stores (store_id, store_name, description, store_image_id, cover_image_id, cbu)
       VALUES ($1, $2, $3, $4, $5, $6)
...
stores.put('/:id', authMiddleware, sellerMiddleware, async (c) => {
    try {
        ...
        // Verify store exists
        const storeCheck = await pool.query('SELECT store_id FROM stores WHERE store_id = $1', [storeId]);
        ...
        values.push(storeId);
        const result = await pool.query(`UPDATE stores SET ${updateFields.join(', ')}
       WHERE store_id = $${paramCount}
       RETURNING store_id, store_name, description, store_image_id, cover_image_id, cbu`, values);
        return c.json({
...
stores.get('/me/store', authMiddleware, sellerMiddleware, async (c) => {
    try {
       ...
       LEFT JOIN images si ON s.store_image_id = si.id
       LEFT JOIN images ci ON s.cover_image_id = ci.id
       WHERE s.store_id = $1`, [user.id]);
        if (result.rows.length === 0) {
            return c.json({ error: 'Store not found' }, 404);
        }
        // Get store statistics
        const productsCountResult = await pool.query('SELECT COUNT(*) FROM products WHERE seller_id = $1 AND deleted = false', [user.id]);
        const activeProductsCountResult = await pool.query('SELECT COUNT(*) FROM products WHERE seller_id = $1 AND deleted = false AND paused = false', [user.id]);
        const store = result.rows[0];
        store.stats = {
