...
products.get('/', async (c) => {
    try {
        ...
        const result = await pool.query(`SELECT p.id, p.name, p.description, p.category, p.price, p.paused,
       ...
       LIMIT $${paramCount} OFFSET $${paramCount + 1}`, values);
        // Get total count
        const countResult = await pool.query(`SELECT COUNT(*) FROM products p ${whereClause}`, values.slice(0, -2));
...
products.get('/:id', async (c) => {
    try {
       ...
       WHERE p.id = $1 AND p.deleted = false`, [productId]);
        if (result.rows.length === 0) {
            return c.json({ error: 'Product not found' }, 404);
        }
        // Get reviews
        const reviewsResult = await pool.query(`SELECT r.id, r.description, r.rating, r.timestamp,
...
products.post('/', authMiddleware, sellerMiddleware, async (c) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO products (name, description, category, seller_id, image_id, price)
       VALUES ($1, $2, $3, $4, $5, $6)
...
products.put('/:id', authMiddleware, sellerMiddleware, async (c) => {
    try {
        ...
        const productCheck = await pool.query('SELECT id FROM products WHERE id = $1 AND seller_id = $2 AND deleted = false', [productId, user.id]);
        ...
        const result = await pool.query(`UPDATE products SET ${updateFields.join(', ')}
       WHERE id = $${paramCount} AND seller_id = $${paramCount + 1} AND deleted = false
       RETURNING id, name, description, category, price, seller_id, image_id, paused`, values);
        return c.json({
...
products.delete('/:id', authMiddleware, sellerMiddleware, async (c) => {
    try {
        ...
        await pool.query('UPDATE products SET deleted = true WHERE id = $1', [productId]);
...
products.patch('/:id/pause', authMiddleware, sellerMiddleware, async (c) => {
    try {
        ...
        // Verify product belongs to user
        const productCheck = await pool.query('SELECT id, paused FROM products WHERE id = $1 AND seller_id = $2 AND deleted = false', [productId, user.id]);
        if (productCheck.rows.length === 0) {
            return c.json({ error: 'Product not found or access denied' }, 404);
        }
        const currentPaused = productCheck.rows[0].paused;
        const newPaused = !currentPaused;
        await pool.query('UPDATE products SET paused = $1 WHERE id = $2', [newPaused, productId]);
        return c.json({
...
products.get('/seller/me', authMiddleware, sellerMiddleware, async (c) => {
    try {
        ...
        const limit = parseInt(c.req.query('limit') || '20');
        const offset = (page - 1) * limit;
        const result = await pool.query(`SELECT id, name, description, category, price, paused, deleted
       FROM products
       WHERE seller_id = $1
       ORDER BY id DESC
       LIMIT $2 OFFSET $3`, [user.id, limit, offset]);
        // Get total count
        const countResult = await pool.query('SELECT COUNT(*) FROM products WHERE seller_id = $1', [user.id]);
        const total = parseInt(countResult.rows[0].count);
        const totalPages = Math.ceil(total / limit);
        return c.json({
            products: result.rows,
